...
def test_sso221():
    ...
    build_config = buildconfigurations.create_build_configuration(
                                                                  ...
                                                                  project=project.id,
                                                                  environment=1, 
                                                                  scm_repo_url="git+ssh://user-pnc-gerrit@pnc-gerrit.pnc.dev.eng.bos.redhat.com:29418/pnc/org.keycloak-keycloak-parent-2.2.1.Final-redhat-1.git",
                                                                  scm_revision="branch-2.2.1.Final-redhat-1-pnc",
...
def test_eap701ga():
    ...
    eap_name = "org.jboss.eap-jboss-eap-parent-7.0.1.GA-redhat-2" + sufix
    build_config = buildconfigurations.create_build_configuration(
                                                                  name=eap_name,
                                                                  project=project.id,
                                                                  environment=1,
                                                                  scm_repo_url="git+ssh://user-pnc-gerrit@pnc-gerrit.pnc.dev.eng.bos.redhat.com:29418/pnc/org.jboss.eap-jboss-eap-parent-7.0.1.GA-redhat-2.git",
                                                                  scm_revision="branch-7.0.1.GA-redhat-2-pnc",
                                                                  build_script="mvn clean deploy -Prelease")

    set = buildconfigurationsets.create_build_configuration_set(name=eap_name)
    buildconfigurationsets.add_build_configuration_to_set(set_id=set.id, config_id=build_config.id)
    build_record = buildconfigurationsets.build_set(id=set.id)
    assert build_record is not None
    print set.id
