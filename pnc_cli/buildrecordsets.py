...
def list_sets_for_build_record(id):
    ...
    """
    response = utils.checked_api_call(brs_api, 'get_all_for_build_record', record_id=id)
    if response: return response.content
...
def list_build_record_sets_for_milestone(id):
    ...
    """
    response = utils.checked_api_call(brs_api,'get_all_for_product_milestone', version_id=id)
    if response: return response.content
