...
def _license_exists(id):
    response = LicensesApi(utils.get_api_client()).getSpecific(id=id)
    if response.ok:
        return True
    return False
...
def delete_license(id):
    if id:
        response = LicensesApi(utils.get_api_client()).delete(id=id)
        if response.ok:
...
def update_license(id, name=None, content=None, reference_url=None, abbreviation=None, project_ids=None):
    ...
    if id:
        if _license_exists(id):
            response = LicensesApi(utils.get_api_client()).update(id=id,body=updated_license)
            if response.ok:
